...
- 顶部按钮手动切换浅色或深色模式，并记住用户选择
- 抽题可优先覆盖未回答题目，标签支持按现有题库搜索和多选
- AI 批改时锁定提交内容、支持取消，并提供紧凑的流式状态预览
- 批改将题库答案作为参考而非唯一标准，可选生成不依赖参考答案的 AI 独立解析并继续追问
- 支持按最近得分筛选低分错题，多次作答完整保留并可对比进步
- 刷题进度、当前位置和逐题草稿自动保存，切换页面或刷新后可以继续
- 文档导入采用标题分类、LLM 语义边界、滑动窗口和全局审计，抑制漏题、误拆与重复题
- 内置可复制给 Agent 的复杂文档处理指令和 JSON 示例
